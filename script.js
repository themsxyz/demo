...
const webAppUrl = "https://script.google.com/macros/s/AKfycbxThwvc7mNgy1lvqgt-JRpDQh9BIeB9i9_NtXfNL7cww342LTAnvoZkV-ihahf6t4Rw/exec";
...
// Class dropdown options - corrected spelling
const classOptions = ["Play", "Nursery", "KG", "1", "2", "3", "4", "5"];

// Account fields – father/mother optional, with hints
const accountFieldsMeta = [
  ...
  { 
    id: "father", 
    label: "পিতার নাম (ঐচ্ছিক)", 
    type: "text", 
    icon: "fas fa-user-tie", 
    validation: "uppercaseOptional", 
    noPlaceholder: true,
    hint: "শুধুমাত্র বড় হাতের অক্ষর (ঐচ্ছিক)"
  },
  { 
    id: "mother", 
    label: "মাতার নাম (ঐচ্ছিক)", 
    type: "text", 
    icon: "fas fa-female", 
    validation: "uppercaseOptional", 
    noPlaceholder: true,
...
function buildAccountForm() {
  ...
  accountFieldsMeta.forEach(field => {
    ...
    if (field.hint) {
      hintSpan.innerHTML = `<i class="fas fa-info-circle"></i> ${field.hint}`;
    } else if (field.validation === "name") {
      hintSpan.innerHTML = "<i class='fas fa-info-circle'></i> প্রথম অক্ষর বড় হাতের হবে";
    } else if (field.validation === "uppercase" || field.validation === "uppercaseOptional") {
      hintSpan.innerHTML = "<i class='fas fa-arrow-up'></i> শুধুমাত্র বড় হাতের অক্ষর";
    } else if (field.validation === "roll") {
      hintSpan.innerHTML = "<i class='fas fa-sort-numeric-up'></i> সর্বোচ্চ ৩ ডিজিট";
    } else if (field.validation === "studentId") {
      hintSpan.innerHTML = "<i class='fas fa-key'></i> সর্বোচ্চ ৬ অক্ষর (ST-XXX)";
    } else if (field.validation === "mobile") {
      hintSpan.innerHTML = "<i class='fas fa-mobile-alt'></i> ১১ ডিজিট দিন (+88 স্বয়ংক্রিয়)";
    } else if (field.id === "cls") {
      hintSpan.innerHTML = "<i class='fas fa-school'></i> শ্রেণি নির্বাচন করুন";
    }
...
function attachAccountFormatters() {
  ...
  if (fatherField) {
    fatherField.addEventListener('input', e => {
      if (e.target.value) {
  ...
  if (motherField) {
    motherField.addEventListener('input', e => {
      if (e.target.value) {
        e.target.value = e.target.value.toUpperCase();
      }
    });
...
function createAccount() {
  ...
  accountFieldsMeta.forEach(f => {
    ...
    // For father/mother, allow empty string; already handled by value being ""
    if ((f.id === "father" || f.id === "mother") && !value) {
      value = "";
    }
...
async function printReceipt() {
  ...
  try {
    ...
    function buildStudentRows() {
      return [
        ['নাম', student.name || '—'],
        ['পিতার নাম', student.father || '—'],
        ['মাতার নাম', student.mother || '—'],
    ...
    function buildFeeItems() { 
      ...
      feeSections.forEach((key, idx) => { 
        const amt = fees[key]; 
        if(amt>0 || key==='tuition'){ 
          items.push([feeLabels[idx], amt + ' Tk']); 
          total += amt; 
        } 
      }); 
      return { items, total }; 
    ...
    const createReceiptColumn = (copyName) => {
      ...
      const { items, total } = buildFeeItems();
      return { stack: [ 
        { text: 'WESTERN SCHOOL AND COLLEGE', style: 'schoolHeader', alignment: 'center' }, 
        { text: 'Didar Market-Chawkbazar, Chittagong', alignment: 'center', fontSize: 8, margin: [0,0,0,4] }, 
        ...
        { text: `Fee taking date & time: ${currentDateTime}`, alignment: 'center', margin: [0,0,0,2] }, 
        { table: { widths: ['auto','*'], body: studentRows }, layout: 'noBorders', margin:[0,0,0,12] }, 
        { table: { widths: ['*', 'auto'], body: [[{ text: 'Description', style: 'tableHeader' }, { text: 'Amount', style: 'tableHeader' }], ...items, [{ text: 'Total', style: 'totalLabel' }, { text: total + ' Tk', style: 'totalAmount' }]] }, layout: 'lightHorizontalLines', margin:[0,0,0,12] }, 
    ...
    const docDefinition = { 
      ...
      content: [ { columns: [createReceiptColumn('School Copy'), createReceiptColumn('Bank Copy'), createReceiptColumn('Guardian Copy')], columnGap: 8 } ], 
      styles: { 
